...
function doPut(uri, req, res) {
	...
	if (utils.startsWith(uri, "/topicviews")) {
		...
		updateObject(utils.substringAfter(uri, "/objects/"), req, res);
	} else {
		res.writeHead(404);
		res.end();
	}
...
/* NJM2 CRUD für 'Objekt' */

/*
 * the methods for handling requests for object
 */
function readObject(uri, req, res) {
	console.log("readObject(): " + uri);

	// the uri segment that we will get passed here is the id of the topic
	if (uri.length > 0) {
		// try to read out the object element from the db
		db.objects.find({
			_id : require("mdbjs").ObjectId(uri) 		// NJM3 (1)
		}, function(err, elements) {

			if (err || !elements) {
				console.log("Error accessing object!",err,elements);
				respondError(res);
			} else if (elements.length == 0) {
				console.error("no object element could be found.",elements);
				respondError(res, 404);
			} else {
				// we only use the first element we find (there shouldn't be more elements, though...)
	...
	/*
	 * in FRM hinzugefügt
	 * if no uri is specified we read out all object
	 */
	else {
		console.log("reading out all object...");
		// try to read out the object element from the db
		db.objects.find(function(err, elements) {
			if (err || !elements) {
				console.log("Error accessing object!");
				respondError(res);
			} else if (elements.length == 0) {
				console.error("no object element could be found.");
				respondError(res, 404);
			} else {
				// we only use the first element we find (there shouldn't be more elements, though...)
				console.log("readObject(): found " + elements.length + " elements.");
...
function createObject(uri, req, res) {
	console.log("createObject(): " + uri);

	var contentType = req.headers["content-type"];
	console.log("createObject(): Content-Type is: " + contentType);
	/*
	 * in FRM hinzugefügt
	 * if we do not have json data, respond an error
	 */
	if (contentType == null || contentType.indexOf("application/json") == -1) {
		console.log("createObject(): cannot process content type: " + req.headers["Content-Type"]);
		respondError(res, 400);
	}
	...
	req.on("end", function() {
		...
		// parse the data
		var parseddata = JSON.parse(alldata);
		// and save it to the objects collection
		db.objects.save(parseddata, function(err, saved) {
			if (err || !saved) {
				console.error("topic data could not be saved: " + err);
				respondError(res);
			} else {
...
function updateObject(uri, req, res) {
	...
	req.on("end", function() {
		...
		// parse the data
		var parseddata = JSON.parse(alldata);
		// and update it to the objects collection - note that we can directly pass the data received to the update function
		db.objects.update({
			_id : require("mdbjs").ObjectId(uri)
		}, {
			$set : parseddata
		}, function(err, updated) {
			if (err || !updated) {
...
function deleteObject(uri, req, res) {
	...
	db.objects.remove({
		...
		if (err || !update) {
			console.log("object " + uri + " could not be deleted. Got: " + err);
			respondError(res);
		} else {
			console.log("object " + uri + " was deleted. Got: " + update);
			respondSuccess(res, update);
		}
	});
}
